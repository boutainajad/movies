...
export default class AppUI {
        ...
        try {
            ...
            results.forEach((result, index) => {
                if (result.status === 'fulfilled' && result.value && result.value.results && result.value.results.length > 0) {
                    ...
                    this.renderRow(HOME_GENRES[index].title, result.value.results, sectionsContainer);
                }
            });
        } else {
            ...
            if (this.state.currentView === 'home') {
                data = await TMDB.getTrending(this.state.currentType, this.state.page, signal);
            } else {
                const params = {
                ...
                data = await TMDB.discover(this.state.currentType, params, signal);
            }
    ...
    switchPage(page) {
        ...
        if (this._gridTitle) this._gridTitle.innerHTML = PAGE_TITLES[page] || 'Résultats';
    ...
    async performSearch(query, page = 1) {
        ...
        try {
            if (page === 1) {
                ...
                if (this._gridTitle) this._gridTitle.textContent = `Résultats pour "${query}"`;
                if (this._grid) this._grid.innerHTML = '';
                const frag = document.createDocumentFragment();
                for (let i = 0; i < SKELETON_COUNT; i++) {
                    const s = document.createElement('div');
